...
tinymce.PluginManager.add('link', function (editor) {
    ...
    function showDialog(linkList) {
        ...
        var win, onlyText, textListCtrl, signDateListCtrl, signPersonIdListCtrl, signKeyListCtrl, signKeyLinkListCtrl, linkListCtrl, relListCtrl, targetListCtrl, classListCtrl, linkTitleCtrl, value;
        var signPersons = {
            '101':
            {
                name: "Золина Татьяна Владимировна",
                position: "Ректор"
            }
        };
        ...
        if ((value = dom.getAttrib(anchorElm, 'data-sign-date'))) {
            data.signDate = value;
        }

        if ((value = dom.getAttrib(anchorElm, 'data-sign-person'))) {
            data.signPersonId = value;
        }

        if ((value = dom.getAttrib(anchorElm, 'data-sign-key'))) {
            data.signKey = value;
        }

        if ((value = dom.getAttrib(anchorElm, 'data-sign-key-link'))) {
            data.signKeyLink = value;
        }
        ...
        if (onlyText) {
            ...
            signDateListCtrl = {
                ...
                type: 'textbox',
                size: 40,
                label: 'Дата и время подписи документа',
                onchange: function () {
                    data.signDate = this.value();
                }
            };
            ...
            for (const id in signPersons) {
                if (Object.hasOwnProperty.call(signPersons, id)) {
                    const element = signPersons[id];
                    list.push({
                        text: element.name,
                        value: id
                    });
            ...
            signPersonIdListCtrl = {
                name: 'signPersonId',
                type: 'listbox',
                label: 'Кто подписал документ',
                values: buildListItems(list)
            };
            ...
            signKeyListCtrl = {
                ...
                type: 'textbox',
                size: 60,
                label: 'Ключ подписи документа',
                onchange: function () {
                    data.signKey = this.value();
                }
            };
            ...
            signKeyLinkListCtrl = {
                ...
                type: 'textbox',
                size: 60,
                label: 'Ссылка на файл ключа подписи',
                onchange: function () {
                    data.signKeyLink = this.value();
                }
            };
        }
        ...
        win = editor.windowManager.open({
            ...
            onSubmit: function (e) {
                ...
                function createLink() {
                    ...
                    if (data.signPersonId && Object.hasOwnProperty.call(signPersons, data.signPersonId)) {
                        var person = signPersons[data.signPersonId];

                        if (data.signDate) {
                            tooltip += "<p>Дата и время подписания: " + data.signDate + "</p>";
                        }
                        tooltip += "<p>ФИО подписавшего документ: " + person.name + "</p>";
                        tooltip += "<p>Должность: " + person.position + "</p>";

                        if (data.signKey) {
                            tooltip += "<p>Уникальный ключ: " + data.signKey + "</p>";
                        }
                    }
                    ...
                    if (tooltip) {
                        linkAttrs["data-tooltip"] = tooltip;
                    }

                    if (data.signDate) {
                        linkAttrs["data-sign-date"] = data.signDate;
                    }

                    if (data.signPersonId) {
                        linkAttrs["data-sign-person"] = data.signPersonId;
                    }

                    if (data.signKey) {
                        linkAttrs["data-sign-key"] = data.signKey;
                    }

                    if (data.signKeyLink) {
                        linkAttrs["data-sign-key-link"] = data.signKeyLink;
                    }
